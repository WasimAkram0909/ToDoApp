    ...
    if (this.props.cards.length !== 0) {
      return (
        <React.Fragment>
                  ...
                  {TaskDetails[1].map((Tasksdata, index) => {
                    ...
                    return (
                      <React.Fragment key={index}>
                        <div
                          onMouseEnter={() => {
                            this.DisplayActionsBtns(Tasksdata.taskId);
                          }}
                          onMouseLeave={() =>
                            this.DisplayActionsBtns(Tasksdata.taskId)
                          }
                        >
                          <div
                            className={`ItemContainer ${itemCls}`}
                            key={Tasksdata.taskId}
                          >
                            ...
                            this.state.selectedId === Tasksdata.taskId ? (
                              <div className="editTaskButtons">
                                ...
                                <img
                                  ...
                                  src={Reschedule}
                                  onClick={() =>
                                    this.onRescheduleButtonClick(Tasksdata)
                                  }
                                  alt=""
                                />
                                <img
          ...
          {this.state.error ? (
            ()=>{this.showTostOnWrongReschedule()}
          ) : null}
          {this.state.showToast  ? (
            <Toast
              showUndoOpt={this.state.showUndoOpt}
              showImg={this.state.showImg}
              showToast={this.state}
              undoMyChanges={this.undoMyChanges}
            />
          ) : null}
        </React.Fragment>
      );
